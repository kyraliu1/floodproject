...
legend(x="bottomleft",legend = level_order, col = pal, pch = 15,xpd = TRUE,cex = 0.8)}
...
barplot(totarea$area, names.arg = totarea$flood.risk, col = pal)
...
ggplot(data = areadata,
  ...
  labs(title = "Distribution of Flood Risk in Yolo and Sacramento County\nAcross Urban and Agricultural Land",
       x = "land use type", y = "area of flood risk category", fill = "flood risk type")+
...
# level_order <- c("high risk", "moderate risk", "low risk")
# totalframe$risk <- factor(totalframe$risk, levels = level_order)
par(mar = c(5,5,6,3))
barplot(totalframe$area,names.arg =level_order,col = pal[3],
...
# # modify the factor levels of the fill variable
 areadata$flood.risk <- factor(areadata$flood.risk, levels = level_order)
